...
export default function FarmerLoading() {
  return (
    <div className="px-4 py-6 max-w-2xl mx-auto animate-pulse" aria-busy="true">
      {/* PageHeader-Balken */}
      <div className="h-7 w-44 rounded-lg mb-2" style={{ background: 'var(--border)' }} />
      <div className="h-4 w-60 rounded mb-6" style={{ background: 'var(--app-trough)' }} />

      {/* Karten-Platzhalter */}
      {[0, 1, 2].map((i) => (
        <div key={i} className="rounded-xl border border-border bg-card p-4 mb-3">
          <div className="h-4 w-36 rounded mb-3" style={{ background: 'var(--app-trough)' }} />
          <div className="h-3 w-full rounded mb-2" style={{ background: 'var(--app-chip)' }} />
          <div className="h-3 w-2/3 rounded" style={{ background: 'var(--app-chip)' }} />
        </div>
      ))}
    </div>
  )
