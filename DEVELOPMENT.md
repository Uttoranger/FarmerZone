...
**Die Buchstaben:** L = Datei nicht lesbar, F = Format, S = Server und Verbindung unterbrochen (beide „nochmal versuchen"), B = der Bildspeicher hat abgelehnt, X = ehrlich unbestimmt.
...
- **Der Bauer liest, was war** (`ordneTransferFehler`): „Verbindung unterbrochen" nur bei einem fetch-Netzfehler (TypeError mit einem der bekannten Browser-Wortlaute) oder unserem Abbruch; hat der Bildspeicher geantwortet — Ablehnung wie Ausfall —, „Der Bildspeicher hat das Foto gerade nicht angenommen — bitte später nochmal." `[B…]`; alles andere ist `[X…]`. Maßgeblich ist der letzte Anlauf.
